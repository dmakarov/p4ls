...
namespace p4l {
std::ostream& boost_test_print_type(std::ostream& os, const boost::wave::token_id tok) {
	os << tok;
	return os;
}
}
...
BOOST_AUTO_TEST_CASE(test_numeric_literals)
{
	...
	BOOST_REQUIRE_EQUAL(token, boost::wave::T_NUMBER);
	...
	token = lexer.next();
	BOOST_REQUIRE_EQUAL(token, boost::wave::T_UNKNOWN);
}
...
BOOST_AUTO_TEST_CASE(test_operators)
{
	...
	BOOST_REQUIRE_EQUAL(token, boost::wave::T_IDENTIFIER);
	BOOST_REQUIRE_EQUAL(value, 0);
	BOOST_REQUIRE_EQUAL(width, 32);
	BOOST_REQUIRE_EQUAL(text, "abc");
	token = lexer.next();
	BOOST_REQUIRE_EQUAL(token, boost::wave::T_BIT_AND);
	token = lexer.next();
	value = lexer.get_value();
	width = lexer.get_width();
	BOOST_REQUIRE_EQUAL(token, boost::wave::T_NUMBER);
	...
	BOOST_REQUIRE_EQUAL(width, 32);
	token = lexer.next();
	BOOST_REQUIRE_EQUAL(token, boost::wave::T_MASK);
	token = lexer.next();
	value = lexer.get_value();
	width = lexer.get_width();
	BOOST_REQUIRE_EQUAL(token, boost::wave::T_NUMBER);
	...
	BOOST_REQUIRE_EQUAL(width, 8);
	token = lexer.next();
	BOOST_REQUIRE_EQUAL(token, boost::wave::T_END);
}
